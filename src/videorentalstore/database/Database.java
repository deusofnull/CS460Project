...
public class Database {
    ...
    public void initDatabaseTables() throws Exception {
        executeUpdate("CREATE TABLE \"customer\" (\"customerId\" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE , \"firstName\" VARCHAR NOT NULL , "
                + "\"lastName\" VARCHAR NOT NULL , \"email\" VARCHAR NOT NULL , \"password\" VARCHAR NOT NULL , \"birthday\" VARCHAR, "
                + "\"creditCardNum\" CHAR(16), \"creditCardExpDate\" VARCHAR, \"address\" VARCHAR, \"city\" VARCHAR, \"state\" VARCHAR, \"zipcode\" CHAR(5))");
    ...
    public void addUsertoDB(User user) {
        ...
        try {
            String insert = "INSERT INTO customer (lastName , firstName , email, password, birthday, creditCardNum, creditCardExpDate, address, city, state, zipCode) VALUES ( "  + " ' " + user.getLastName() + " ' " + " , " + " ' " + user.getFirstName() + " ' " + " , " + " ' " + user.getEmail() + " ' " + " , " + " ' " + user.getPassword() + " ' " +
               " , " +  " ' " + user.getBirthday() + " ' " + " , " + " ' " + user.getCreditCardNum() + " ' " + " , " + " ' " + user.getCreditCardExpireDate() + " ' " + " , " +    
    ...
    public void addUsertoDBshort(User user) {
        //Statement stmt;
        boolean userAddRES;
        try {
            String insert = "INSERT INTO customer (lastName , firstName , email, password) VALUES ( " +  " ' " + user.getLastName() + " ' " + " , " + " ' " + user.getFirstName() + " ' " + " , " + " ' " + user.getEmail() + " ' " + " , " + " ' " + user.getPassword() + " ' " + " ) ";
            System.out.println(insert);
            stmt = conn.createStatement();
            userAddRES = stmt.execute(insert);
        }
    ...
    public void searchUserinDBbyLastName(String lastName){
        try {
            ...
            while (searchUserLastNameRES.next()) {
                String user = "ID:" + searchUserLastNameRES.getString("customerID") +  "First Name: " + searchUserLastNameRES.getString("firstName") + "Last Name: " +searchUserLastNameRES.getString("lastName") + "Email Address" + searchUserLastNameRES.getString("email") + "Password: " + searchUserLastNameRES.getString("password");
    ...
    public void deleteUserbyID(int ID){
        ...
        try {
            ...
            stmt = conn.createStatement();
            userDeleteRES = stmt.execute(delete);
            System.out.println("User: " + ID + " was deleted from the database.");
        }
        catch (Exception e){
            System.out.println("Error deleting user from Database: " + e.toString());
            try {
